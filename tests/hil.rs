...
mod hil_tests {
    ...
    use sensor_swarm::testing::hil::{init_hil_test_sync, test_led_basic, test_device_info};
    use sensor_swarm::hw::traits::{DeviceManagement, Led, BackupRegisters};
    ...
    fn test_hil_led_operations() {
        ...
        // Create LED
        let mut led = ctx.device.create_led()
            ...
            Ok(mut backup_regs) => {
                ...
                for (index, &value) in test_values.iter().enumerate() {
                    if index < backup_regs.register_count() {
                        ...
                        defmt::assert_eq!(read_value, value, 
                            "Backup register {} mismatch: wrote 0x{:08X}, read 0x{:08X}", 
                            index, value, read_value);
